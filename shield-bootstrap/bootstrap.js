...
/* here is some shieldy stuff */
//const shield = Cu.import('/path/to/shield');
const shield = {} //
const studyConfig = {
  name: 'aName',
  days: 1,
...
const study = new shield.ShieldStudy(studyConfig);

const feature = {
  startup: function (aStyle) {},
}
...
this.install = function () {
  if (study.isEligible()) {
    ...
    study.setVariation(variation).persist();  // choose and save
  } else {
    die() //
  }
};
...
this.startup = function (data, reason) {
  study.load().watchExpiration();  // revive, then set watchers

};

this.shutdown = function (data, reason) {
  study.shutdown(reason);  // sends telemetry, asks surveys
};
