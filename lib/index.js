...
// this whole approach fails for 'undefined' prefs, b/c
// can't set a pref to undefined.
// which means we can't know that the pref is shadows, b/c isset is bad.
// TODO, find a solution that lasts across sessions.  Maybe it's indexedDb?
var shadowPref = {
  "set":  function setShadowPref (name, value) {
  ...
  "original":  function (name) {return prefSvc.get(name + ".shadow")},
  "reset":  function resetShadowPref (name) {
    let shadow = name + ".shadow";
    if (prefSvc.isSet(shadow)) {
  ...
  "isShadowed": function (name) {
    let shadow = name + ".shadow";
    return prefSvc.isSet(shadow)
  }
};
  ...
  initialize: function initialize(xconfig, variationsMod) {
    ...
    this.on(
      "ineligible-die", (function () {
        ...
        report(merge({}, this.xconfig, {msg:"ineligible"}));
        this.final();
        die();
      }).bind(this)
    )
    ...
    this.on(
      "normal-shutdown", (function () {
        ...
        report(merge({}, this.xconfig, {msg:"shutdown"}));
        this.final();
      }).bind(this)
    )
    this.on(
