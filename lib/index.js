...
function expired (xconfig, now = Date.now() ) {
  console.log("expired??", xconfig, now);
...
function handleStartup (startupOptions, thisStudy) {
  ...
  */
  ...
  thisStudy._pulseTimer = setInterval(thisStudy.alivenessPulse, 5*60*1000 /*5 minutes */)
  return thisStudy;
}

function handleOnUnload (reason, thisStudy) {
  // https://developer.mozilla.org/en-US/Add-ons/SDK/Tutorials/Listening_for_load_and_unload
  if (thisStudy.flags.ineligibleDie) { return thisStudy } // special case.
