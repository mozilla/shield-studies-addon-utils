...
// sdk
const { Class } = require("sdk/core/heritage"); // For now, eventually es6
...
function xsetup (xSetupConfig) {
  ...
  */
  return {
    variation: chooseOrReviveVariation(xSetupConfig.choices),
...
var Study = Class({
  ...
  initialize: function initialize(config) {
    ...
    //merge(this, options);
    this.config = config; // TODO: should this be copy?
    this.flags = {
    ...
    this.on(
      "ineligible-die", (function () {
        ...
        report(merge({}, telemetrySubset(this.config), {study_state: "ineligible"}));
        this.final();
        die();
      }).bind(this)
    )
    this.on(
      "installing", (function () {
        if (!this.config.isEligible()) {
          ...
          report(merge({}, telemetrySubset(this.config), {study_state: "install"}));
          this.changeState("modifying");
        }
      }).bind(this)
    )
    ...
    this.on(
      "running", (function () {
        // report success
        report(merge({}, telemetrySubset(this.config), {study_state: "running"}));
        this.final();
      }).bind(this)
    )
    this.on(
      "normal-shutdown", (function () {
        ...
        report(merge({}, telemetrySubset(this.config), {study_state: "shutdown"}));
        this.final();
      }).bind(this)
    )
    this.on(
      "end-of-study", (function () {
        if (this.flags.expired) {
          ...
          report(merge({}, telemetrySubset(this.config) ,{study_state: "end-of-study"}));
          // survey for end of study
          survey(this.config, {"reason": "end-of-study"});
          this.cleanup();
          resetPrefs();
          this.final();
          die();
    ...
    this.on(
      "user-uninstall-disable", (function () {
        ...
        this.flags.dying = true;
        report(merge({}, telemetrySubset(this.config), {study_state: "user-ended-study"}));
        survey(this.config, {"reason": "user-ended-study"});
        this.cleanup();
        this.final();
        die();
      }).bind(this)
  ...
  startup: function (reason) {
    ...
    */
    ...
    switch (reason) {
      case "install":
        emit(this, "change", "installing");  // TODO merges install and normal startup
        break;
...
// class ShieldStudy {
//     constructor(variationsAndStuff) {
//         // Validate variations against methods
//     }
//
//     lint() {
//         if (!('butts' in this)) {
...
//import ConsoleLogStudy from 'file';
//import { ConsoleLogStudy } from 'file';
//
//export class ConsoleLogStudy extends ShieldStudy {
//    constructor() {
//        super();
//        console.log('Working!');
//    }
...
//
//    shouldInstallStudy() {
//        return userHasConsoleInstall;
//    }
//}


// function test() {
//     study = new ConsoleLogStudy(with thing);
//     study.handleVariation1();
// }
