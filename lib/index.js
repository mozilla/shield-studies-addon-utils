...
class Study extends EventTarget {
  constructor (config) {
    ...
    this.on(
      'normal-shutdown', (function () {
        ...
        //this._telemetry({study_state: "shutdown"}, "shield-study");
        this.final();
      }).bind(this)
    );
    this.on(
  ...
  telemetry(data, schema={}) {
    ...
    DEBUG(JSON.stringify(validation));
    if (validation.errors.length) {
      let errorReport = {
        'error_id': 'jsonschema-validation-addon-defined',
        'error_source': 'addon',
        'severity': 'fatal',
    ...
    this._telemetry(payload, 'shield-study-addon');
  }
