...
// sdk
const { merge } = require('./not-jetpack');
const { prefs } = require('./not-jetpack');
const { prefSvc } = require('./not-jetpack');
...
var TelemetryWatcher = new EventTarget().on('telemetry',
  (d) => prefSvc.get('shield.debug') && console.log('TELEMETRY',JSON.stringify(d))
);
...
function die (addonId=self.id) {
  ...
  /* istanbul ignore next */
  require('./not-jetpack').uninstall(addonId);
}
...
class Study extends EventTarget {
  constructor (config) {
    ...
    this.on(
      'change', (function (newstate) {
        prefSvc.get('shield.debug') && console.log(`state: "${newstate}". previous: ${this.states}`);
    ...
    this.on(
      'expired', (function () {
        if (this.flags.expired) {  // safe to call multiple times
          ...
          let that = this;
          generateTelemetryIdIfNeeded().then(()=>that.showSurvey('expired'));
    ...
    this.on(
      'user-disable', (function () {
        ...
        let that = this;
        generateTelemetryIdIfNeeded().then(()=>that.showSurvey('user-disable'));
        try {this.cleanup();} catch (err) {/*ok*/} finally { /*ok*/ }
        this.final();
        die();
      }).bind(this)
  ...
  alivenessPulse (ap=new ActivePing()) {
    ...
    let last = ap.get();
    console.log('PPPP pulsing', t, last);
  ...
  telemetry(data, bucket='shield-study-addon') {
    ...
    payload.data.packet = bucket;  // to comply with schemas
    ...
    /* istanbul ignore next */
    try {
      validation = jsonschema.validate(payload, shieldSchema);
    } catch (err) {
    ...
    console.log(JSON.stringify(payload), JSON.stringify(validation));
